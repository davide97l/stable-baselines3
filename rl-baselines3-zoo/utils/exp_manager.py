...
class ExperimentManager(object):
    ...
    def create_callbacks(self):
        ...
        if self.eval_freq > 0 and not self.optimize_hyperparameters:
            ...
            if self.stop_value is None:
                eval_best_callback = SaveVecNormalizeCallback(save_freq=1, save_path=self.params_path)
            else:
                eval_best_callback = StopTrainingOnRewardThreshold(reward_threshold=self.stop_value, verbose=1)
            eval_callback = EvalCallback(
                self.create_envs(self.n_eval_envs, eval_env=True),
                callback_on_new_best=eval_best_callback,
